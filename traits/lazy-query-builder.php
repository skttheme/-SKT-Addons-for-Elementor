...
trait Lazy_Query_Builder {
	...
	protected function register_query_controls() {
		$this->add_control(
			'posts_post_type',
			[
				'label' => __( 'Post Type', 'skt-addons-for-elementor' ),
		...
		$this->add_control(
			'posts_selected_ids',
			[
				'label' => __( 'Search & Select', 'skt-addons-for-elementor' ),
		...
		$this->add_control(
			'posts_include_by',
			[
				'label' => __( 'Include By', 'skt-addons-for-elementor' ),
				'type' => Controls_Manager::SELECT2,
				'multiple' => true,
				'label_block' => true,
				'options' => [
					'authors' => __( 'Authors', 'skt-addons-for-elementor' ),
		...
		$this->add_control(
			...
			[
				'label' => __( 'Authors', 'skt-addons-for-elementor' ),
				'type' => Lazy_Select::TYPE,
				'multiple' => true,
				'label_block' => true,
				'lazy_args' => [
		...
		$this->add_control(
			...
			[
				'label' => __( 'Terms', 'skt-addons-for-elementor' ),
				'description' => __( 'Terms are items in a taxonomy. The available taxonomies are: Categories, Tags, Formats and custom taxonomies.', 'skt-addons-for-elementor' ),
				'type' => Lazy_Select::TYPE,
				'multiple' => true,
				'label_block' => true,
				'placeholder' => __( 'Type and select terms', 'skt-addons-for-elementor' ),
				'lazy_args' => [
		...
		$this->add_control(
			'posts_exclude_by',
			[
				'label' => __( 'Exclude By', 'skt-addons-for-elementor' ),
				'type' => Controls_Manager::SELECT2,
				'multiple' => true,
				'label_block' => true,
				'options' => [
					'authors'           => __( 'Authors', 'skt-addons-for-elementor' ),
					'current_post'      => __( 'Current Post', 'skt-addons-for-elementor' ),
		...
		$this->add_control(
			...
			[
				...
				'default' => 'anytime',
				'options' => [
					'anytime' => __( 'All', 'skt-addons-for-elementor' ),
					'today'   => __( 'Past Day', 'skt-addons-for-elementor' ),
					'week'    => __( 'Past Week', 'skt-addons-for-elementor' ),
					'month'   => __( 'Past Month', 'skt-addons-for-elementor' ),
					'quarter' => __( 'Past Quarter', 'skt-addons-for-elementor' ),
					'year'    => __( 'Past Year', 'skt-addons-for-elementor' ),
					'exact'   => __( 'Custom', 'skt-addons-for-elementor' ),
				],
				'condition' => [
					'posts_post_type!' => 'manual_selection',
				]
		...
		$this->add_control(
			...
			[
				...
				'description' => __( 'Setting a ‘Before’ date will show all the posts published until the chosen date (inclusive).', 'skt-addons-for-elementor' ),
				'condition' => [
		...
		$this->add_control(
			'posts_date_after',
			[
				'label' => __( 'After', 'skt-addons-for-elementor' ),
				'type' => Controls_Manager::DATE_TIME,
				'description' => __( 'Setting an ‘After’ date will show all the posts published since the chosen date (inclusive).', 'skt-addons-for-elementor' ),
				'condition' => [
					'posts_select_date' => 'exact',
					'posts_post_type!' => 'manual_selection',
				]
		...
		$this->add_control(
			...
			[
				...
				'options' => [
					...
					'comment_count' => __( 'Comment Count', 'skt-addons-for-elementor' ),
		...
		$this->add_control(
			...
			[
				...
				'default' => 'desc',
				'options' => [
					'asc' => __( 'ASC', 'skt-addons-for-elementor' ),
					'desc' => __( 'DESC', 'skt-addons-for-elementor' ),
				]
			]
		);
		...
		$this->add_control(
			...
			[
				'label' => __( 'Ignore Sticky Posts', 'skt-addons-for-elementor' ),
		...
		$this->add_control(
			...
			[
				'label' => __( 'With Featured Image', 'skt-addons-for-elementor' ),
				'description' => __( 'Enable to display posts only when featured image is set.', 'skt-addons-for-elementor' ),
				'type' => Controls_Manager::SWITCHER,
				'return_value' => 'yes',
				'condition' => [
					'posts_post_type!' => 'manual_selection'
	...
	protected function setup_query_with_featured_image() {
		...
		if ( $this->get_settings_for_display( 'posts_only_with_featured_image' ) === 'yes' ) {
			$this->query_args['meta_key'] = '_thumbnail_id'; // phpcs:ignore 	WordPress.DB.SlowDBQuery.slow_db_query_meta_key
		}
	}
	...
	protected function get_query_param_by( $by = 'exclude' ) {
		$by_map = [
			'exclude' => 'posts_exclude_by', // phpcs:ignore 	WordPressVIPMinimum.Performance.WPQueryParams.PostNotIn_exclude
	...
	protected function setup_query_exclude_post() {
		...
		if ( in_array( 'manual_selection', $exclude_by ) ) {
			...
			$this->query_args['post__not_in'] = array_merge( $current_post, wp_parse_id_list( $exclude_ids ) ); // phpcs:ignore 	WordPressVIPMinimum.Performance.WPQueryParams.PostNotIn_post__not_in
		}
	}
	...
	protected function setup_query_taxonomy() {
		...
		if ( ! empty( $terms_query ) ) {
			$this->query_args['tax_query'] = $terms_query; // phpcs:ignore 	WordPress.DB.SlowDBQuery.slow_db_query_tax_query
