...
trait Button_Renderer {
	...
	public function render_icon_button( $args = [] ) {
		...
		if ( $button_text && ( empty( $has_new_icon ) && empty( $has_old_icon ) ) ) :
			printf( '<a %1$s>%2$s</a>',
				wp_kses_post($this->get_render_attribute_string( 'button' )), 
				sprintf( '<span %1$s>%2$s</span>', wp_kses_post($this->get_render_attribute_string( $args['text'] )), esc_html( $button_text ) )
			);
