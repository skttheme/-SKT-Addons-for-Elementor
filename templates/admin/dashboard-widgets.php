...
<div class="skt-dashboard-panel">
    <div class="skt-dashboard-panel__header">
        <div class="skt-dashboard-panel__header-content">
            <p class="f16"><?php /* translators: translate widget text */ printf( esc_html__( 'Here is the list of all %1$s widgets. You can enable or disable widgets from here to optimize loading speed and Elementor editor experience. %2$sAfter enabling or disabling any widget make sure to click the Save Changes button.%3$s', 'skt-addons-for-elementor' ), esc_attr($total_widgets_count), '<strong>', '</strong>' ); ?></p>

            <div class="skt-action-list">
                <button type="button" class="skt-action--btn" data-action="enable"><?php esc_html_e( 'Enable All', 'skt-addons-for-elementor' ); ?></button>
                <button type="button" class="skt-action--btn" data-action="disable"><?php esc_html_e( 'Disable All', 'skt-addons-for-elementor' ); ?></button>
            </div>
        </div>
    </div>
			...
			foreach( $catwise_widgets as $cat => $widgets) :
				if( $widgets ):
					...
					printf('<h2 %s>%s %s</h2><br>',
						...
						esc_attr__( 'Widgets', 'skt-addons-for-elementor' )
					);
					foreach ( $widgets as $widget_key => $widget_data ) :
						...
						<div class="<?php echo esc_attr($class_attr); ?>">
							<?php if ( $is_pro ) : ?>
								<span class="skt-dashboard-widgets__item-badge"><?php esc_html_e( 'Pro', 'skt-addons-for-elementor' ); ?></span>
							<?php endif; ?>
							...
							<h3 class="skt-dashboard-widgets__item-title">
								...
								<?php if ( $demo_url ) : ?>
									<a href="<?php echo esc_url( $demo_url ); ?>" target="_blank" rel="noopener" data-tooltip="<?php esc_attr_e( 'Click to view demo', 'skt-addons-for-elementor' ); ?>" class="skt-dashboard-widgets__item-preview"><i aria-hidden="true" class="eicon-device-desktop"></i></a>
								<?php endif; ?>
							</h3>
    ...
    <div class="skt-dashboard-panel__footer">
        <button disabled class="skt-dashboard-btn skt-dashboard-btn--save" type="submit"><?php esc_html_e( 'Save Settings', 'skt-addons-for-elementor' ); ?></button>
    </div>
</div>