...
class Select2_Handler {
	...
	protected static function validate_reqeust() {
		$nonce = sanitize_text_field(wp_unslash(! empty( $_REQUEST['nonce'] ) ? $_REQUEST['nonce'] : ''));
	...
	public static function process_post() {
		...
		$query_term   = sanitize_text_field(wp_unslash(! empty( $_REQUEST['query_term'] ) ? $_REQUEST['query_term'] : ''));
		$saved_values = sanitize_text_field(wp_unslash(! empty( $_REQUEST['saved_values'] ) ? $_REQUEST['saved_values'] : 0));
	...
	public static function process_mailchimp_list() {
		...
		// Resolved nonce varification issue
		if ( sanitize_text_field(wp_unslash(isset( $_POST['mailchimp_api_choose_nonce'] ))) && wp_verify_nonce( sanitize_text_field(wp_unslash($_POST['mailchimp_api_choose_nonce'], 'mailchimp_api_choose_nonce_action' ))) ) {
	        $choose_api = sanitize_text_field(wp_unslash(! empty( $_REQUEST['mailchimp_api_choose'] ) ? $_REQUEST['mailchimp_api_choose'] : ''));
	    } else {
	        $choose_api = sanitize_text_field(wp_unslash(! empty( $_REQUEST['mailchimp_api_choose'] ) ? $_REQUEST['mailchimp_api_choose'] : ''));
	    }

		$global_api = sanitize_text_field(wp_unslash(! empty( $_REQUEST['global_api'] ) ? $_REQUEST['global_api'] : ''));
		$custom_api = sanitize_text_field(wp_unslash(! empty( $_REQUEST['mailchimp_api'] ) ? $_REQUEST['mailchimp_api'] : $global_api));
