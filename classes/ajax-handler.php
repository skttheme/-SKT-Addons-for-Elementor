...
class Ajax_Handler {
	...
	public static function smart_post_list_ajax() {
		...
		if (true == $security && isset($_POST['querySettings'])) :
			...
			if (isset($_POST['offset'])) {
				$args['offset'] = sanitize_text_field(wp_unslash($_POST['offset']));
			}
			if (isset($_POST['termId']) && is_numeric(isset($_POST['termId']))) {
				$args['tax_query'] = array( // phpcs:ignore 	WordPress.DB.SlowDBQuery.slow_db_query_tax_query
					array(
						'taxonomy' => '',
						'field' => 'term_taxonomy_id',
						'terms' => sanitize_text_field(wp_unslash($_POST['termId'])),
					),
				);
			}
	...
	public function post_grid_ajax() {
		...
		if (true == $security && isset($_POST['querySettings'])) :
			...
			$settings = sanitize_text_field(wp_unslash(isset($_POST['querySettings'])));
			$loaded_item = sanitize_text_field(wp_unslash(isset($_POST['loadedItem'])));
    ...
    public static function post_tab() {
		...
		if (true == $security) :
			$settings   =  wp_unslash(isset($_POST['querySettings']));

			$term_id = absint(wp_unslash(isset($_POST['term_id'])));
	...
	public static function mailchimp_prepare_ajax() {
        ...
        parse_str(sanitize_text_field(wp_unslash(isset($_POST['subscriber_info']) ? $_POST['subscriber_info'] : '', $subsciber)));
	...
	public static function edd_show_product_quick_view() {
		$nonce = sanitize_text_field(wp_unslash(! empty( $_GET['nonce'] ) ? $_GET['nonce'] : ''));
		...
		<div class="skt-pqv-edd">
			...
			<div class="skt-pqv-edd__content">
				...
				<div class="skt-pqv-edd__summary"><?php echo esc_attr(wp_trim_words( get_the_content( $post ), 100 )); ?></div>
				<div class="skt-pqv-edd__cart">
					...
					if( edd_has_variable_prices( $post->ID ) ){
						...
						printf( '<a href="%s" class="button" target="_blank">%s</a>',
							esc_url( get_the_permalink( $post->ID ) ),
						 	esc_attr__( 'Buy Now', 'skt-addons-for-elementor' )
						);
					}
					?>
				</div>
