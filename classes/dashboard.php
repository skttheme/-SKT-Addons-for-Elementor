...
class Dashboard {
    ...
    public static function is_page() {
        return ( isset( $_GET['page'] ) && ( $_GET['page'] === self::PAGE_SLUG || $_GET['page'] === self::LICENSE_PAGE_SLUG ) ); // phpcs:ignore  WordPress.Security.NonceVerification.Recommended 
    }
    ...
    public static function add_action_links( $links ) {
        ...
        $links = array_merge( [
            sprintf( '<a href="%s">%s</a>',
                ...
                esc_html__( 'Settings', 'skt-addons-for-elementor' )
            )
        ], $links );
    ...
    public static function save_data() {
        ...
        $posted_data = wp_unslash(! empty( $_POST['data'] ) ? $_POST['data'] : '');
        $data = [];
    ...
    public static function enqueue_scripts( $hook ) {
		...
		if($screen->id == 'dashboard') {
			wp_enqueue_style(
				'skt-addons-for-elementor-wp-dashboard',
        ...
        wp_enqueue_style(
            'skt-addons-for-elementor-dashboard',
        ...
        wp_localize_script(
            ...
            [
                ...
                'saveChangesLabel' => esc_html__( 'Save Changes', 'skt-addons-for-elementor' ),
                'savedLabel' => esc_html__( 'Changes Saved', 'skt-addons-for-elementor' ),
            ]
        );
    }
    ...
    public static function add_menu() {
		self::$menu_slug = add_menu_page(
			__( 'SKT Addons for Elementor', 'skt-addons-for-elementor' ),
			__( 'SKT Addons', 'skt-addons-for-elementor' ),
			'manage_options',
			self::PAGE_SLUG,
        ...
        if ( is_array( $tabs ) ) {
            foreach ( $tabs as $key => $data ) {
                ...
                add_submenu_page(
                    self::PAGE_SLUG,
					/* translators: translate sub menu page heading text */
                    sprintf( __( '%s - SKT Addons for Elementor', 'skt-addons-for-elementor' ), $data['title'] ),
                    $data['title'],
                    'manage_options',
                    self::PAGE_SLUG . '#' . $key,
                    [ __CLASS__, 'render_main' ]
    ...
    public static function get_tabs() {
        $tabs = [
            'home' => [
                'title' => esc_html__( 'Home', 'skt-addons-for-elementor' ),
                'renderer' => [ __CLASS__, 'render_home' ],
            ],
            'widgets' => [
                'title' => esc_html__( 'Widgets', 'skt-addons-for-elementor' ),
                'renderer' => [ __CLASS__, 'render_widgets' ],
            ],
            'features' => [
                'title' => esc_html__( 'Features', 'skt-addons-for-elementor' ),
                'renderer' => [ __CLASS__, 'render_features' ],
            ],
            'credentials' => [
                'title' => esc_html__( 'Credentials', 'skt-addons-for-elementor' ),
                'renderer' => [ __CLASS__, 'render_credentials' ],
            ],
            'analytics' => [
                'title' => esc_html__( 'Analytics', 'skt-addons-for-elementor' ),
