...
class Lazy_Query_Manager {
	...
	protected static function get_query_handler() {
		...
		if ( sanitize_text_field(wp_unslash(isset( $_POST['query_nonce'] ))) && wp_verify_nonce( sanitize_text_field(wp_unslash($_POST['query_nonce'], 'query_nonce_action' ))) ) {
	...
	protected static function get_post_type() {
		// Resolved nonce varification issue
		if ( sanitize_text_field(wp_unslash(isset( $_POST['post_type'] ))) && wp_verify_nonce( sanitize_text_field(wp_unslash($_POST['post_type'], 'post_type_action' ))) ) {
	        $query = sanitize_text_field(wp_unslash(isset( $_POST['post_type'] ) ?  $_POST['post_type']  : ''));
	    } else {
	        $query = sanitize_text_field(wp_unslash(isset( $_POST['post_type'] ) ?  $_POST['post_type']  : ''));
	    }
		return $post_type;
