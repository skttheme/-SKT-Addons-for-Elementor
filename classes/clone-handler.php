...
class Clone_Handler {
	...
	public static function add_row_actions( $actions, $post ) {
		if ( self::can_clone() && post_type_supports( $post->post_type, 'elementor' ) ) {
			$actions[ self::ACTION ] = sprintf(
				...
				/* translators: translate clone text */
				sprintf( esc_attr__( 'Clone - %s', 'skt-addons-for-elementor' ), esc_attr( $post->post_title ) ),
				esc_html__( 'SKT Clone', 'skt-addons-for-elementor' )
			);
		}
	...
	public static function duplicate_thing() {
		...
		// Resolve unslashed sanitization issue
		...
		$nonce = sanitize_text_field(wp_unslash(isset( $_GET['_wpnonce'] ) ? $_GET['_wpnonce'] : ''));
		$post_id = sanitize_text_field(wp_unslash(isset( $_GET['post_id'] ) ? absint( $_GET['post_id'] ) : 0));
		$ref = sanitize_text_field(wp_unslash(isset( $_GET['ref'] ) ? $_GET['ref'] : ''));
	...
	protected static function duplicate_post( $post ) {
		...
		$duplicated_post_args = [
			...
			/* translators: translate clone text */
			'post_title'     => sprintf( __( '%1$s - [Cloned #%2$d]', 'skt-addons-for-elementor' ), $post->post_title,
				$post->ID ),
		];
	...
	protected static function duplicate_meta_entries( $post, $duplicated_post_id ) {
		...
		$entries = $wpdb->get_results( // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching, WordPress.DB.PreparedSQL.NotPrepared 
			 "SELECT meta_key, meta_value FROM {$wpdb->postmeta} WHERE post_id = %d", $post->ID  // 
		);
