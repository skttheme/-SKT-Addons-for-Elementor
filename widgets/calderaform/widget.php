...
class CalderaForm extends Base {
    ...
    public function get_title() {
        return __( 'Caldera Forms', 'skt-addons-for-elementor' );
    }
	...
	protected function register_content_controls() {
		...
		$this->start_controls_section(
			'_section_calderaf',
			[
				'label' => skt_addons_elementor_is_calderaforms_activated() ? __( 'Caldera Forms', 'skt-addons-for-elementor' ) : __( 'Missing Notice', 'skt-addons-for-elementor' ),
			]
		);
        ...
        if ( ! skt_addons_elementor_is_calderaforms_activated() ) {
            $this->add_control(
                ...
                [
                    ...
                    'raw' => sprintf(
                        /* translators: translate raw text */ __( 'Hello %2$s, looks like %1$s is missing in your site. Please click on the link below and install/activate %1$s. Make sure to refresh this page after installation or activation.', 'skt-addons-for-elementor' ),
			...
			$this->add_control(
				...
				[
					...
					'label_block' => true,
					'options' => [''] + \skt_addons_elementor_get_caldera_form(),
				]
			);
    ...
    protected function __fields_style_controls() {
		...
		$this->start_controls_tab(
            'tab_field_focus',
            [
                'label' => __( 'Focus 123', 'skt-addons-for-elementor' ),
            ]
		);
    ...
    protected function render() {
        if ( ! skt_addons_elementor_is_calderaforms_activated() ) {
			skt_addons_elementor_show_plugin_missing_alert( __( 'Caldera Form', 'skt-addons-for-elementor' ) );
            return;
        }
        ...
        if ( ! empty( $settings['form_id'] ) ) {
            ...
            echo esc_attr(skt_addons_elementor_do_shortcode( 'caldera_form', [ 'id' => $settings['form_id'], ] ));
		}
    }
}