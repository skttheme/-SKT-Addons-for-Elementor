...
class Image_Grid extends Base {
	...
	public function get_title() {
		return __( 'Image Grid', 'skt-addons-for-elementor' );
	}
	...
	protected function __gallery_content_controls() {
		...
		$this->start_controls_section(
			'_section_gallery',
			[
				'label' => __( 'Gallery', 'skt-addons-for-elementor' ),
		...
		$repeater->add_control(
			'filter',
			[
				'label' => __( 'Filter Name', 'skt-addons-for-elementor' ),
				'type' => Controls_Manager::TEXT,
				'placeholder' => __( 'Type gallery filter name', 'skt-addons-for-elementor' ),
				'description' => __( 'Filter name will be used in filter menu.', 'skt-addons-for-elementor' ),
				'default' => __( 'Filter Name', 'skt-addons-for-elementor' ),
				'dynamic' => [
					'active' => true,
				]
			]
		...
		$repeater->add_control(
			'is_default_filter',
			[
				'label' => __( 'Is Default Filter?', 'skt-addons-for-elementor' ),
				'type' => Controls_Manager::SWITCHER,
				'return_value' => 'yes',
				'description' => __( 'Set this as default active filter. Make sure filter menu is active and visible. Last active will get priority.', 'skt-addons-for-elementor' ),
				'style_transfer' => true,
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'title_field' => sprintf( __( 'Filter Group: %1$s', 'skt-addons-for-elementor' ), '{{filter}}' ),
				'default' => [
					[
						'filter' => __( 'SKT', 'skt-addons-for-elementor' ),
					]
				]
			]
		);
	...
	protected function __advance_content_controls() {
		...
		$this->start_controls_section(
			'_section_advance',
			[
				'label' => __( 'Advance', 'skt-addons-for-elementor' ),
				'tab'   => Controls_Manager::TAB_CONTENT,
			]
		);
		...
		$this->add_control(
			'show_filter',
			[
				'label' => __( 'Show Filter Menu', 'skt-addons-for-elementor' ),
				'type' => Controls_Manager::SWITCHER,
				'return_value' => 'yes',
				'description' => __( 'Enable to display filter menu.', 'skt-addons-for-elementor' ),
				'style_transfer' => true,
			]
		);
		...
		$this->add_control(
			'show_all_filter',
			[
				'label' => __( 'Show "All" Filter', 'skt-addons-for-elementor' ),
				'type' => Controls_Manager::SWITCHER,
				'return_value' => 'yes',
				'default' => 'yes',
				'description' => __( 'Enable to display "All" filter in filter menu.', 'skt-addons-for-elementor' ),
				'condition' => [
					'show_filter' => 'yes'
				],
				'style_transfer' => true,
		...
		$this->add_control(
			'all_filter_label',
			[
				'label' => __( 'Filter Label', 'skt-addons-for-elementor' ),
				'type' => Controls_Manager::TEXT,
				'default' => __( 'All', 'skt-addons-for-elementor' ),
				'placeholder' => __( 'Type filter label', 'skt-addons-for-elementor' ),
				'description' => __( 'Type "All" filter label.', 'skt-addons-for-elementor' ),
				'condition' => [
					'show_all_filter' => 'yes',
					'show_filter' => 'yes'
				],
		...
		$this->add_responsive_control(
			...
			[
				...
				'options' => [
					1 => __( '1 Column', 'skt-addons-for-elementor' ),
					2 => __( '2 Columns', 'skt-addons-for-elementor' ),
					3 => __( '3 Columns', 'skt-addons-for-elementor' ),
					4 => __( '4 Columns', 'skt-addons-for-elementor' ),
					5 => __( '5 Columns', 'skt-addons-for-elementor' ),
					6 => __( '6 Columns', 'skt-addons-for-elementor' ),
				],
				'separator' => 'before',
				'desktop_default' => 4,
				'tablet_default' => 3,
		...
		$this->add_control(
			...
			[
				...
				'type' => Controls_Manager::SELECT,
				'options' => [
					'even' => __( 'Even', 'skt-addons-for-elementor' ),
					'fitRows' => __( 'Fit Rows', 'skt-addons-for-elementor' ),
					'masonry' => __( 'Masonry', 'skt-addons-for-elementor' ),
				],
				'default' => 'masonry',
				'render' => 'none',
	...
	protected function __image_style_controls() {
		...
		$this->add_control(
			...
			[
				...
				'description' => __( 'Image height is only applicable for Even layout', 'skt-addons-for-elementor' ),
	...
	protected function __menu_style_controls() {
		...
		$this->start_controls_section(
			'_section_style_menu',
			[
				'label' => __( 'Filter Menu', 'skt-addons-for-elementor' ),
				'tab'   => Controls_Manager::TAB_STYLE,
			]
		);
		...
		$this->add_control(
			'_heading_menu',
			[
				'label' => __( 'Menu', 'skt-addons-for-elementor' ),
		...
		$this->add_control(
			'_heading_buttons',
			[
				'label' => __( 'Filter Buttons', 'skt-addons-for-elementor' ),
				'type' => Controls_Manager::HEADING,
				'separator' => 'before',
			]
		);
	...
	protected function image_missing_alert() {
		if( skt_addons_elementor()->editor ){
			printf(
				...
				esc_attr__( 'Please select an image first to render the grid properly', 'skt-addons-for-elementor' )
			);
		}
	}
	...
	protected function render() {
		...
		if ( $settings['show_filter'] ) : ?>
			<div class="skt-filter sktjs-filter" data-default-filter="<?php echo esc_attr($this->_default_filter); ?>" role="navigation" aria-label="<?php echo esc_attr_x( 'Gallery filter', 'Gallery filter aria label', 'skt-addons-for-elementor' ); ?>">
		...
		if ( \Elementor\Plugin::$instance->editor->is_edit_mode() ) :
			printf( '<script>jQuery(".sktjs-isotope-%s").isotope();</script>', wp_kses_post($this->get_id() ));
		endif;
	}
}