...
class Mailchimp_api {
    ...
    public static function insert_subscriber_to_mailchimp($submitted_data) {
        ...
        $widget_settings = sanitize_text_field(skt_addons_elementor_get_ele_widget_settings(sanitize_text_field(wp_unslash(! empty($_POST['post_id']))), sanitize_text_field(wp_unslash(! empty($_POST['widget_id']))))); // phpcs:ignore WordPress.Security.NonceVerification.Missing 
        ...
        $auth = [
            'api_key' => self::$apiKey,
            'list_id' => sanitize_text_field(wp_unslash(! empty($_POST['list_id']))), // phpcs:ignore WordPress.Security.NonceVerification.Missing 
        ];
        ...
        if(!isset($server[1])) {
            return ['status' => 0, 'msg' => esc_html__('Invalid API key.', 'skt-addons-for-elementor')];
        }
        ...
        if (is_wp_error($response)) {
            ...
            if ($body['status'] > 399 && $body['status'] < 600) {
                ...
                $return['status'] = 1;
                $return['msg'] = esc_html__('Your data inserted on Mailchimp.', 'skt-addons-for-elementor');
            }else {
                $return['status'] = 0;
                $return['msg'] = esc_html__('Something went wrong. Try again later.', 'skt-addons-for-elementor');
            }
        }
