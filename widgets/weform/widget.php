...
class WeForm extends Base {
    ...
    public function get_title() {
        return __( 'weForms', 'skt-addons-for-elementor' );
    }
	...
	protected function register_content_controls() {
		...
		$this->start_controls_section(
			'_section_weforms',
			[
				'label' => skt_addons_elementor_is_weforms_activated() ? __( 'weForms', 'skt-addons-for-elementor' ) : __( 'Missing Notice',
        ...
        if ( ! skt_addons_elementor_is_weforms_activated() ) {
            ...
            $this->add_control(
                ...
                [
                    ...
                    'raw' => sprintf(
                       /* translators: translate raw text */
					    __( 'Hello %2$s, looks like %1$s is missing in your site. Please click on the link below and install/activate %1$s. Make sure to refresh this page after installation or activation.', 'skt-addons-for-elementor' ),
			...
			$this->add_control(
				...
				[
					...
					'label_block' => true,
					'options' => ['' => '' ] + \skt_addons_elementor_get_we_forms(),
				]
			);
    ...
    protected function __label_style_controls() {
		...
		$this->add_group_control(
            ...
            [
                ...
                'label' => __( 'Help Text Typography', 'skt-addons-for-elementor' ),
		...
		$this->add_control(
            'desc_color',
            [
                'label' => __( 'Help Text Color', 'skt-addons-for-elementor' ),
                'type' => Controls_Manager::COLOR,
                'selectors' => [
    ...
    protected function render() {
        if ( ! skt_addons_elementor_is_weforms_activated() ) {
			skt_addons_elementor_show_plugin_missing_alert(__( 'Weforms', 'skt-addons-for-elementor' ) );
            return;
        }
        ...
        /*if ( ! empty( $settings['form_id'] ) ) {
            echo skt_addons_elementor_do_shortcode( 'weforms', [ // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped 
                'id' => $settings['form_id'],
            ] );
		}*/
        
        if ( ! empty( $settings['form_id'] ) ) {
        $shortcode_output = skt_addons_elementor_do_shortcode( 'weforms', [
            'id' => esc_attr( $settings['form_id'] ),
        ]);

        echo wp_kses_post( $shortcode_output ); // Escaping the output
        }
    }
}