...
class Unfold extends Base {
	...
	public function get_title() {
		return __( 'Unfold', 'skt-addons-for-elementor' );
	}
	...
	protected function __register_content_tab() {
		...
		$this->add_control(
			...
			[
				...
				'default'     => __( 'Unfold Magic', 'skt-addons-for-elementor' ),
				'placeholder' => __( 'Type Unfold Title', 'skt-addons-for-elementor' ),
				'dynamic'     => [
					'active' => true,
				],
			]
		...
		$this->add_control(
			...
			[
				...
				'separator' => 'before',
				'options'   => [
					'editor'   => __( 'Editor', 'skt-addons-for-elementor' ),
		...
		$this->add_control(
			'editor',
			[
				'label'      => __( 'Content Editor', 'skt-addons-for-elementor' ),
				'show_label' => false,
		...
		$this->add_control(
			'template',
			[
				'label'       => __( 'Section Template', 'skt-addons-for-elementor' ),
				'placeholder' => __( 'Select a section template for as tab content', 'skt-addons-for-elementor' ),
				'description' => sprintf(
				/* translators: translate decription text */ __( 'Wondering what is section template or need to create one? Please click %1$shere%2$s ', 'skt-addons-for-elementor' ),
	...
	protected function __register_fold_tab() {
		$this->start_controls_section(
			'_section_fold',
			[
				'label' => __( 'Fold/ Unfold Options', 'skt-addons-for-elementor' ),
				'tab'   => Controls_Manager::TAB_CONTENT,
			]
		);
		...
		$this->add_control(
			'trigger_notice',
			[
				'raw'             => '<strong>' . esc_html__( 'Note!', 'skt-addons-for-elementor' ) . '</strong> ' . esc_html__( 'Please disable the button under "Button" section. ', 'skt-addons-for-elementor' ) . '<br>' . esc_html__( 'Having both button & trigger hover will make the button non functioning.', 'skt-addons-for-elementor' ),
		...
		$this->add_responsive_control(
			'fold_height',
			[
				'label'              => __( 'Fold Height (px)', 'skt-addons-for-elementor' ),
		...
		$this->add_control(
			'transition_duration',
			[
				'label'              => __( 'Transition Duration (ms)', 'skt-addons-for-elementor' ),
				'type'               => Controls_Manager::NUMBER,
				'min'                => 1,
				'max'                => 5000,
				'step'               => 1,
	...
	protected function __register_button_tab() {
		...
		$this->add_control(
			'button_disable',
			[
				'label'        => __( 'Disable Button?', 'skt-addons-for-elementor' ),
		...
		$this->add_control(
			'content_position',
			[
				'label'        => __( 'Button Position Above Content?', 'skt-addons-for-elementor' ),
				'type'         => Controls_Manager::SWITCHER,
				'label_on'     => __( 'Yes', 'skt-addons-for-elementor' ),
				'label_off'    => __( 'No', 'skt-addons-for-elementor' ),
				'return_value' => 'yes',
		...
		$this->add_control(
			...
			[
				'type'               => Controls_Manager::TEXT,
				'label'              => __( 'Unfold Text', 'skt-addons-for-elementor' ),
				'default'            => __( 'Read More', 'skt-addons-for-elementor' ),
				'placeholder'        => __( 'Type Unfold Text', 'skt-addons-for-elementor' ),
				'frontend_available' => true,
				'conditions'         => $this->button_condition,
				'dynamic'            => [
					'active' => true,
		...
		$this->add_control(
			'unfold_icon',
			[
				'label'                  => __( 'Unfold Icon', 'skt-addons-for-elementor' ),
		...
		$this->add_control(
			...
			[
				'type'               => Controls_Manager::TEXT,
				'label'              => __( 'Fold Text', 'skt-addons-for-elementor' ),
				'default'            => __( 'Read Less', 'skt-addons-for-elementor' ),
				'placeholder'        => __( 'Type Fold Text', 'skt-addons-for-elementor' ),
				'frontend_available' => true,
				'conditions'         => $this->button_condition,
				'dynamic'            => [
					'active' => true,
		...
		$this->add_control(
			'fold_icon',
			[
				'label'                  => __( 'Fold Icon', 'skt-addons-for-elementor' ),
				'type'                   => Controls_Manager::ICONS,
				'label_block'            => false,
				'skin'                   => 'inline',
				'exclude_inline_options' => ['svg'],
	...
	protected function __register_style_box_tab() {
		...
		$this->add_responsive_control(
			'overlay_height',
			[
				'label'      => __( 'Overlay Height', 'skt-addons-for-elementor' ),
		...
		$this->add_group_control(
			...
			[
				'name'           => 'overlay_color',
				'label'          => esc_html__( 'Overlay Color', 'skt-addons-for-elementor' ),
				'types'          => ['gradient'],
	...
	protected function __register_style_content_tab() {
		...
		$this->add_group_control(
			...
			[
				'name'     => 'content_shadow',
				'label'    => __( 'Shadow', 'skt-addons-for-elementor' ),
				'selector' => '{{WRAPPER}} .skt-unfold-widget-wrapper .skt-unfold-data-render',
			]
		);
	...
	protected function __register_style_button_tab() {
		...
		$this->add_responsive_control(
			...
			[
				'label'      => __( 'Space Between Icon & Text', 'skt-addons-for-elementor' ),
				'type'       => Controls_Manager::SLIDER,
				'size_units' => ['px', '%'],
				'range'      => [
					'px' => [
			...
			<div class="skt-unfold-data">
				<div class="skt-unfold-data-render">
					...
					elseif ( $settings['source'] === 'template' && $settings['template'] ) :
						echo skt_addons_elementor()->frontend->get_builder_content_for_display( $settings['template'] );  // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
					endif;
					?>
				</div>
			</div>
