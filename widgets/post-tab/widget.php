...
class Post_Tab extends Base {
	...
	public function get_title () {
		return __( 'Post Tab', 'skt-addons-for-elementor' );
	}
	...
	protected function __query_content_controls () {
		...
		foreach ( self::get_post_types() as $key => $value ) {
			...
			$this->add_control(
				'tax_type_' . $key,
				[
					'label' => __( 'Taxonomies', 'skt-addons-for-elementor' ),
					'type' => Controls_Manager::SELECT,
			...
			foreach ( $taxonomy[$key] as $tax_key => $tax_value ) {
				...
				$this->add_control(
					'tax_ids_' . $tax_key,
					[
						'label' => __( 'Select ', 'skt-addons-for-elementor' ) . $tax_value,
						'label_block' => true,
						'type' => Select2::TYPE,
						'multiple' => true,
						'sortable' => true,
	...
	protected function __settings_content_controls () {
		...
		$this->add_responsive_control(
			...
			[
				...
				'options' => [
					...
					'2' => __( '2 Column', 'skt-addons-for-elementor' ),
					'3' => __( '3 Column', 'skt-addons-for-elementor' ),
					'4' => __( '4 Column', 'skt-addons-for-elementor' ),
					'5' => __( '5 Column', 'skt-addons-for-elementor' ),
					'6' => __( '6 Column', 'skt-addons-for-elementor' ),
				],
				'desktop_default' => '4',
				'tablet_default' => '3',
				'mobile_default' => '1',
		...
		$this->add_control(
			'filter_pos',
			[
				'label' => __( 'Filter Position', 'skt-addons-for-elementor' ),
				'label_block' => false,
		...
		$this->add_control(
			'filter_align',
			[
				'label' => __( 'Filter Align', 'skt-addons-for-elementor' ),
				'label_block' => false,
				'type' => Controls_Manager::CHOOSE,
				'default' => 'left',
				'options' => [
		...
		$this->add_responsive_control(
			'event',
			[
				'label' => __( 'Tab action', 'skt-addons-for-elementor' ),
				'type' => Controls_Manager::SELECT,
				'options' => [
					'click' => __( 'On Click', 'skt-addons-for-elementor' ),
					'hover' => __( 'On Hover', 'skt-addons-for-elementor' ),
				],
				'default' => 'click',
				'render_type' => 'template',
				'style_transfer' => true,
	...
	protected function __tab_style_controls () {
		...
		$this->start_controls_section(
			...
			[
				'label' => __( 'Tab', 'skt-addons-for-elementor' ),
		...
		$this->add_responsive_control(
			'tab_item',
			[
				'label' => __( 'Tab Item', 'skt-addons-for-elementor' ),
	...
	protected function render () {
		...
		$term_id = isset($filter_list[0]->term_id) ? $filter_list[0]->term_id : '';
		// Resolved slow query issue
		if ($term_id) {
							...
							<div class="skt-post-tab-item">
								<div class="skt-post-tab-item-inner">
									...
									<?php
										...
										printf(
											'<%1$s class="skt-post-tab-title"><a href="%2$s">%3$s</a></%1$s>',
											esc_html( $title_tag ),
									...
									<?php if( 'yes' === $settings['excerpt'] ): ?>
										<div class="skt-post-tab-excerpt">
											<?php 
											// Resolved escaping issue
											printf( '<p>%1$s</p>', esc_attr(skt_addons_elementor_pro_get_excerpt( get_the_ID(), 20 )) ); ?>
										</div>
									<?php endif;?>
								</div>
							</div>
		...
		<?php
			...
			printf( '%1$s',
				esc_attr__( 'No  Posts  Found', 'skt-addons-for-elementor' )
			);
		endif;
	}
}